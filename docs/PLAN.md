...
3. **Wire codec `openai_chat`**: parse/encode requests, responses, chunk streams, errors; serve `/v1/chat/completions`, `/v1/models`. (`/v1/completions` and `/v1/embeddings` were planned here but are **not implemented** — no route is registered for either; tracked as AUDIT #213.)
