// Public layout — minimal top-nav for the unauthenticated front of the site
// (landing, models catalog, docs). Reuses the admin dark surface + ambient
// background tokens so the public pages share the same visual language as the
// console without pulling in the admin sidebar.

import { Link, NavLink, Outlet } from "react-router-dom";
import { Boxes, BookOpen, Terminal } from "lucide-react";
...
export function PublicLayout() {
  return (
    <div data-admin className="relative z-0 min-h-screen bg-[var(--admin-bg)]">
      {/* Ambient background: faint grid + radial glows (mirrors AdminLayout) */}
      ...
      <header className="admin-topbar sticky top-0 z-30">
        <div className="mx-auto flex h-[64px] max-w-[1400px] items-center gap-6 px-6">
          <Link to="/" className="flex items-center gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-gradient-to-br from-brand-500 to-fuchsia-600 font-mono text-sm font-bold text-white ring-1 ring-white/[0.06] ring-inset">
              w
            </span>
            <div>
      ...
      {/* Footer */}
      <footer className="relative z-10 border-t border-[var(--admin-border)] py-6">
        <div className="mx-auto max-w-[1400px] px-6 text-center text-[11px] text-[var(--admin-text-dim)]">
          wiwi · self-hosted LLM gateway
        </div>
      </footer>
    </div>
  );
